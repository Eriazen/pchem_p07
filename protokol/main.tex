...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \item Zahrnutí lidského faktoru a náhody v podobě reakčního času a náhodných změn rychlosti vozidla o malé hodnoty a implementace simulace v periodických okrajových podmínkách odpovídajích experimentu viz~\cite{sugiyama2008}.
    ...
    Model simulace byl implementován v programovacím jazyce Python, konkrétně knihovně \textit{Manim}~\cite{manim2025}. Ačkoliv je tato knihovna primárně určena pro tvorbu matematických animací, její schopnost efektivně pracovat s 2D vektorovou grafikou a časovými osami ji činí vhodnou i pro vizualizaci kinematických úloh.
    ...
    \end{equation}
    Fyzická vzdálenost je poté dána jako $s_\alpha = \Delta \theta \cdot R - L_{\alpha}$. V této implementaci poslední vozidlo vždy navazuje na první a systém se chová jako nekonečná kolona.
    ...
    \begin{table}[H]
		\begin{center}
			...
			\begin{tabular}
                ...
                Počet aut $N$ & 25\\
                Maximální rychlost $v_{\mathrm{max}}$ & \SI{36}{\meter\per\second}\\
    ...
    V rámci tohoto projektu byla vytvořena a analyzována simulace fantomové dopravní zácpy s využitím modelu IDM. Simulace byla implementována v jazyce Python s využitím knihovny \textit{Manim}~\cite{manim2025}, která umožnila jednoduchou a názornou vizualizaci problému viz~\cite{source2025}.
